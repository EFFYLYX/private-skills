...
description: |
  ...
  Supports list, status, push, pull, and publish commands.
  Triggers on: "private skills", "list skills", "push skills",
  "pull skills", "publish skill", "skill status", "sync skills".
user_invocable: true
argument-hint: "<list|status|push|pull|publish> [skill_name]"
---
...
For a plain (non-git) skill directory: initializes git, commits all files, and creates a private GitHub repo under `EFFYLYX/<skill_name>` using `gh repo create`.
